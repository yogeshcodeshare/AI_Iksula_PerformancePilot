...
export function ExecutiveSummaryCard({
  ...
  return (
    <Card className="rounded-xl shadow-sm border-border mb-8 bg-gradient-to-br from-secondary to-card overflow-hidden">
      <div className="border-b border-border px-6 py-4 flex items-center gap-2">
        <ShieldCheck className="h-4 w-4 text-blue-600" />
        <h2 className="text-xs font-bold tracking-widest text-muted-foreground uppercase">Executive Summary</h2>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-sm text-muted-foreground">page-device pairs pass CWV</span>
            </div>
          </div>

          {/* Top Critical Page */}
          <div className="flex flex-col gap-1">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Attention Required</p>
            {topCriticalPage ? (
              <div className="mt-1">
                <p className="font-semibold text-foreground text-sm">{topCriticalPage.pageLabel}</p>
                <p className="text-xs text-red-600 font-medium">
                  {metrics.filter(m => m.pageId === topCriticalPage.pageId && m.status === 'poor').length} poor metric(s)
                </p>
                <p className="text-xs text-muted-foreground font-mono mt-0.5 truncate">{topCriticalPage.url}</p>
              </div>
            ) : (
          ...
          {/* Highest Risk Metric */}
          <div className="flex flex-col gap-1">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Highest Risk Metric</p>
            {highestRiskMetric ? (
              <div className="mt-1">
